// work_seam_weld_control_ui.c
#include <lvgl/lvgl.h>
#include <stdio.h>
#include <string.h>
#include "work_seam_weld_control_ui.h"
...
static void return_btn_event(lv_event_t* e) {
    ...
    work_destroy_seam_weld_control_ui();
}
...
static lv_obj_t* work_create_top_title(lv_obj_t* parent) {
    ...
    lv_label_set_text(label, "缝焊控制方式设置");
    lv_obj_center(label);
...
static void work_create_welder_type_display(lv_obj_t* parent) {
    ...
    lv_label_set_text(welder_type_value, "缝焊");
...
// 创建完整的点焊控制方式设置界面
static void work_create_seam_weld_control_interface(void) {
...
// 销毁界面
void work_destroy_seam_weld_control_ui(void) {
    if (scr) {
        lv_obj_del(scr);
        scr = NULL;
    }
...
// 初始化UI
void work_seam_weld_control_ui_init(void) {
    printf("work_seam_weld_control_ui_init run !!!\n");
    work_create_seam_weld_control_interface();
}

...
static void param_init(RKADK_PLAYER_FRAME_INFO_S *pstFrmInfo)
{
    ...
    pstFrmInfo->u32DispWidth = RK_PCT_H(57);
#if USE_RK3506
    pstFrmInfo->u32DispHeight = RK_PCT_W(96);
#else
...
#endif
    pstFrmInfo->u32VoLay = 0; // rkadk select the default first device
    pstFrmInfo->u32VoChn = 1; // ui is 2 . play is 1
    pstFrmInfo->u32VoDev = 0; // rkadk select the default first device
...
static void rkadk_init(void)
{
    ...
    stPlayCfg.stFrmInfo.u32FrmInfoX = RK_PCT_H(33);
    stPlayCfg.stFrmInfo.u32FrmInfoY = RK_PCT_W(2);

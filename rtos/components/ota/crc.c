...
static const UINT gTable_Crc32[256] =
{
...
#define tt2 2*tt
static UINT s[tt2 + 1]; // Syndrome values

static UINT rr;//redundant length        // BCH code parameters


static UINT p[mm + 1];
static UINT alpha_to[nn + 1], index_of[nn + 1] ; // Galois field
static UINT gg[rr_max + 1] ;      // Generator polynomial
...
static UINT ggx4 = 0;
// get crc32 value
UINT CRC_32(unsigned char *pData, UINT ulSize, UINT uiPreviousValue)
{
    UINT i;
...
static UINT gen_roots[nn + 1];
static UINT gen_roots_true[nn + 1] ;     // Roots of generator polynomial

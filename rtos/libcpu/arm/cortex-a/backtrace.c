...
int unwind_frame(struct stackframe *frame, const struct unwind_idx **origin_idx, const struct unwind_idx exidx_start[], const struct unwind_idx exidx_end[])
{
    ...
    {
        if (rt_hw_cpu_id() == 0)
            ctrl.sp_high = (unsigned long)((char *)&stack_start + RT_IRQ_STACK_PGSZ);
        else
            ctrl.sp_high = (unsigned long)((char *)&sub_stack_top - rt_hw_cpu_id() * SUB_ISR_Stack_Size + SUB_RT_IRQ_STACK_PGSZ);
    }
#else
    if (rt_interrupt_nest == 0)
        ...
        ctrl.sp_high = (unsigned long)((char *)&stack_start + RT_IRQ_STACK_PGSZ);
#endif
    LOG_D("%s(pc = %08lx lr = %08lx sp = %08lx sp_high = %08lx)\n", __func__,
          frame->pc, frame->lr, frame->sp, ctrl.sp_high);

    idx = unwind_find_idx(frame->pc, origin_idx, exidx_start, exidx_end);
    if (!idx)
    {
        LOG_W("unwind: Index not found %08lx\n", frame->pc);
        return -URC_FAILURE;
    }
...
void unwind_backtrace(struct pt_regs *regs, const struct unwind_idx exidx_start[], const struct unwind_idx exidx_end[])
{
    ...
    LOG_D("stack_start:%x stack_top:%x sub_stack_start:%x sub_stack_top:%x\n", &stack_start, &stack_top, &sub_stack_start, &sub_stack_top);
    LOG_D("%s(regs = %x)\n", __func__, regs);

    arm_get_current_stackframe(regs, &frame);

#ifndef RT_BACKTRACE_FUNCTION_NAME
    rt_kprintf("please use: arm-none-eabi-addr2line -e rtthread.elf -a -f %08x", frame.pc);
#endif
    LOG_D("pc = %08x, sp = %08x\n", frame.pc, frame.sp);
